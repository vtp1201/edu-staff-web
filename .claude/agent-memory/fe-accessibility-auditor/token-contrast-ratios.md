...
## Additional ratios confirmed (US-E24.4 cross-subject-tabs audit, 2026-09-05)
- text-edu-primary-accessible (#4468E0) on bg-edu-primary-light (#ECF2FF) at
  10.5px font-extrabold: 4.35:1 — FAILS 4.5:1 (10.5px extrabold does not clear
  the ≥14px-bold large-text threshold). Recurring shape: `#4468E0` passes on
  WHITE (4.88:1, confirmed US-E12.4/E13.7) but FAILS on the lighter
  `--edu-primary-light` tint — always re-check the actual adjacent bg, don't
  assume "passes on white" transfers to a tinted bg. Fix used: swap to
  `text-foreground` (#2A3547, 11.5:1) — same fix StatusBadge already applies
  for primary/info/purple/teal tones.

## Additional ratios confirmed (US-E24.5 course-player audit, 2026-09-05)
- text-edu-primary-accessible (#4468E0) on bg-primary/12 blended tint (~#EBF0FF): 4.31:1 — FAILS 4.5:1 for 12px bold "active row" label text (not large text). Fix: text-foreground (#2A3547) on same tint = 10.9:1 PASS.
- text-edu-warning-text (#9A6A0F) on --edu-warning-light (#FEF5E5) at 12px bold: 4.37:1 — FAILS (confirms US-E12.4's documented "bold ≥14px only" calibration; 12px still needs 4.5, only 14px+ bold qualifies as large-text 3:1 floor). Fix: bump to text-sm(14px) or swap to --edu-warning-foreground.
- Confirmed `--muted-foreground` IS aliased to `--edu-text-secondary` (#5A6A85, 5.48:1) in light mode per globals.css (ADR referenced in CLAUDE.md) — do NOT flag `text-muted-foreground` as a contrast fail anymore; it's safe in both light and dark mode (dark: 5.85:1, already recorded). Older recurring-failures.md entries about text-muted-foreground predate this alias fix.
