...
## ARIA — Radix `Tabs` used WITHOUT `TabsContent` leaves `aria-controls` dangling
Pattern: When a screen reuses one shared list re-rendered per active filter (rather than a separate panel per tab) and skips `TabsContent` entirely, `@radix-ui/react-tabs`'s `TabsTrigger` STILL unconditionally sets `aria-controls={makeContentId(baseId, value)}` (verified in `node_modules/@radix-ui/react-tabs/dist/index.mjs`) — that id points at an element that was never rendered. This is a broken ARIA reference (WCAG 4.1.2) and means the visible list region has no `role="tabpanel"`/`aria-labelledby` pairing to the active tab, so AT users get no programmatic "this is the tab's content" association even though sighted/keyboard users see it update correctly.
Fix: wrap the shared list in `<TabsContent value={activeTab} forceMount>` (or one `TabsContent` per tab value, all sharing the same child render) so Radix's own `id`/`aria-labelledby`/`role="tabpanel"` wiring resolves; don't hand-roll a plain `<div>` under a `Tabs`/`TabsList` when the content itself isn't also a `TabsContent`.
Seen in: US-E24.13 notification-dropdown.tsx (bell dropdown tablist — Tất cả/Chưa đọc/Hệ thống share one list div, no TabsContent).

## Focus — Radix `FocusScope` autofocuses the FIRST tabbable child on open, not "the intended first interactive region"
Pattern: A popover/dialog's written AC says focus should land on a specific region (e.g. "mở → focus vào tablist") but the panel also renders an EARLIER-in-DOM-order optional action (e.g. a conditional "mark all read" link before the tablist). Radix's `FocusScope` (used by `Popover.Content`/`Dialog.Content`) autofocuses the first focusable descendant in DOM order by default — with no explicit `onOpenAutoFocus` override, focus lands on that earlier optional element whenever it's present (e.g. `unreadCount > 0`), not on the tablist, silently deviating from the written AC in exactly the state most likely to be manually tested (has unread items).
Fix: either reorder the DOM so the intended landing element is first, or add an explicit `onOpenAutoFocus={(e) => { e.preventDefault(); tabRef.current?.focus(); }}` on the Content. Always write a Storybook interaction assertion for `document.activeElement` immediately after open (not just after close/Escape) when an AC specifies where initial focus should land.
Seen in: US-E24.13 notification-dropdown.tsx (header's "Đánh dấu đã đọc" button, when rendered, precedes the `Tabs` in DOM order — no story asserts post-open focus target).

## Status message — Optimistic count/state change with no `aria-live`, while a sibling screen already has the pattern
Pattern: A compact/dropdown variant of an already-shipped full-page feature (e.g. a bell-dropdown preview of a notifications centre) re-implements the mutation logic (optimistic mark-read, unread-count decrement) but drops the full-page sibling's `role="log" aria-live="polite"` wrapper around the list and the `role="status"` announcement pattern for count changes. Visually the badge/pill number updates and the row loses bold+dot, but a screen-reader user with focus still on the clicked row hears nothing confirming "marked as read, N unread remaining" — success is silent (only the `onError` path gets a toast, which sonner's own live region does announce).
Fix: copy the sibling's exact wrapper (`<div role="log" aria-live="polite" aria-label={t("listAriaLabel")}>` around the item list) into the new compact variant, and/or add a `role="status"` sr-only span that updates with the new unread count on successful mark-read/mark-all-read (not just on error).
Seen in: US-E24.13 notification-dropdown.tsx (no aria-live anywhere in the file) vs. notifications-center.tsx lines ~175/215 (has `role="alert"`/`role="log" aria-live="polite"` already, in the same feature).

## Touch target — Inline text-only action button/link inside a header row has no min-height
Pattern: A secondary text action rendered as a bare `<button className="text-xs font-bold ...">` (no size/padding utility) sitting in a flex row that only has `pt-3` (no `pb-`/`py-`) on the wrapper. The clickable box collapses to the line-height of the text (~16–20px), well under the repo's 44×44px touch-target floor — even though a SIBLING footer link in the same component correctly has `min-h-11`.
Fix: add `min-h-11` (and ideally `px-2 -mx-2` to keep the visual text unchanged while growing the hit area) to any inline text-button/link that isn't already inside a 44px-tall row with full-row click handling.
Seen in: US-E24.13 notification-dropdown.tsx "Đánh dấu đã đọc" (mark-all-read) button — the footer "Xem tất cả thông báo" `Link` two elements below DOES have `min-h-11`, showing the omission is inconsistent within the same file.
...
## A11y — aria-invalid conflated with client-side length guard, fires before any user interaction
Pattern: a reject/validation textarea computes `const valid = value.trim().length >= MIN_LEN` and sets `aria-invalid={!valid || Boolean(serverErrorKey)}`. Since the panel auto-focuses the field on mount and `value` starts empty, `aria-invalid=true` is announced to screen readers from the very first render — before the user has typed anything. This conflates two spec-distinct layers (client UX hint that just disables Confirm vs. the server-driven inline error that legitimately needs aria-invalid+aria-describedby).
Fix: drive `aria-invalid` ONLY from `Boolean(serverErrorKey)` (the real, post-submit validation failure); keep the length hint as plain non-ARIA descriptive text.
Seen in: US-E09.5 sd-reject-panel.tsx (and the identical pre-existing bug in staff-leave-request-card.tsx:238 — same shared reject-panel shape, not yet fixed there either).

## Focus — Row action-button group unmounts after a successful mutation, no focus restoration, despite an existing sibling-feature fix for the identical shape
Pattern: a row's Approve/Reject/Submit buttons are rendered conditionally on caller-computed `canDecide`/`canSubmit` (correctly absent-not-disabled for role/state reasons). When a mutation succeeds and the row's state changes, these booleans flip false and the whole button group unmounts with no focus management — focus silently drops to `<body>`. The identical row shape already has the WCAG 2.4.3 fix ONE feature over (`staff-leave-request-card.tsx:68-78`, "Return focus to the reject trigger when the inline panel closes") that was not replicated in the new feature; sometimes a `useRef` for the trigger is even declared and attached but never `.focus()`'d (dead ref) — a strong signal the pattern was half-copied.
...
## Contrast — Shared ChildIdentityHeader avatar-initials text fails at 12px for both tone variants
Pattern: A promoted shared component (`ChildIdentityHeader`) hardcodes `text-xs` (12px) on `AvatarFallback` regardless of the `size` prop (md 38px / lg 48px avatar), then colors initials with self-hue text-on-own-tint: `tone="primary"` → `bg-primary/10 text-primary` (`--primary` = `--edu-primary-dark` #4570EA ≈ 4.41:1, barely fails 4.5:1); `tone="purple"` → `bg-edu-purple/15 text-edu-purple` (#7B5EA7 on its own /15 tint ≈ 4.32:1, same known purple-tone near-miss recorded elsewhere in this file). Neither `font-bold`(700)/`font-semibold`(600) at 12px qualifies as "large text" (needs ≥14px bold per this repo's own accessibility.md rule), so both need the full 4.5:1, not 3:1.
Fix: swap to the already-existing accessible variants — `text-edu-primary-accessible` (#4468E0, 4.88:1) for primary tone, `text-edu-purple-text` (#5B3D8A) for purple tone. Because this is a *shared* component (promoted from 2 inline copies to 3 consumers in one story), the fix propagates to all consumers at once — same high-leverage lesson as `detail-panel-header.tsx`.
Seen in: US-E20.4 child-identity-header.tsx `identityToneClass()` (consumed by children-overview-screen's ChildOverviewCard, parent-dashboard.tsx, child-consent-card.tsx).

## Link — Single-`<Link>`-card keyboard claim verified real via Storybook `userEvent.tab()` — good precedent to look for
Pattern: when a story claims "whole card is one tab stop, verified by Storybook test," check the actual play function calls `userEvent.tab()` N times and asserts `toHaveFocus()` on each card in sequence (not just `getAllByRole("link")` count). This is genuine proof, distinct from just reading the JSX and inferring keyboard behavior from a single `<Link>` wrapper.
Seen in: US-E20.4 children-overview-screen.stories.tsx `KeyboardReachesCard` (real, confirmed pattern — no finding, just a positive precedent to check for on future "single card = one Link" claims).
...
Seen in: US-E09.5 create-violation-dialog.tsx:295, set-conduct-note-dialog.tsx:253 (sd-reject-panel.tsx:57 in the same story correctly used motion-safe: for its expand animation).

## Focus — Radix Dialog inside components/ui/sheet/sheet.tsx STILL has no onCloseAutoFocus override (DEF-01 fixed in AlertDialog-based DestructiveConfirmDialog, NOT in Sheet)
Pattern: `DestructiveConfirmDialog` (AlertDialog-based) was patched with a `document.activeElement` snapshot + `onCloseAutoFocus={returnFocus}` (confirmed present, US-E18.32 audit) — but `components/ui/sheet/sheet.tsx`'s `SheetContent` has NO equivalent fix, and no consumer-level override either. Any `<Sheet open={bool} onOpenChange={...}>` triggered from a row/list button with no `<SheetTrigger>` (e.g. detail panels opened from a table row's "open detail" icon button) drops focus to `<body>` on close instead of returning to the row that opened it.
Fix: same pattern as `DestructiveConfirmDialog` — capture `document.activeElement` in a ref when the sheet transitions `open: false→true`, pass `onCloseAutoFocus={(e) => { e.preventDefault(); ref.current?.focus(); }}` to `SheetContent` (fix once in the shared `sheet.tsx` primitive so all consumers inherit it, don't patch per-screen).
Seen in: US-E18.32 report-detail-sheet.tsx (opened via ReportTable/ReportCardList row buttons, no SheetTrigger anywhere in the tree; moderation-screen.stories.tsx has no play-function test asserting focus returns to the triggering row on close).

## Contrast — StatCard shared primitive: icon color = raw tone (e.g. text-edu-warning/text-edu-success) on its own /15 icon-box background
Pattern: `components/shared/stat-card/stat-card.tsx`'s `TONE` map colors the icon itself with the raw hue (`text-edu-warning` #FFAE1F, `text-edu-success` #13DEB9, etc.) on a `bg-edu-{tone}/15` box — same self-hue-on-own-tint failure recorded elsewhere for badges/text, but here it's the icon (needs 3:1 as a UI/graphical object, WCAG 1.4.11). Warning ~1.7:1, success ~1.56:1 on their own tints — both fail badly. This is a PRE-EXISTING, unmodified shared component (not introduced by the consuming story) — any new `<StatCard tone="warning"|"success"|... icon={...} />` call site inherits it "for free."
Fix (primitive-level, propagates to every consumer): swap `TONE[tone].icon` to the AA-safe `-text` variants already used elsewhere in this same file for the compact variant (`text-edu-warning-text`? — verify one exists; `text-edu-success-text`/`text-edu-error-text` already confirmed elsewhere) or `text-edu-text-primary` if no `-text` warning token clears 3:1 at this weight/size.
Seen in: US-E18.32 stat-row.tsx (`<StatCard tone="warning" .../>` for pending, `<StatCard tone="success" .../>` for resolved) — flagged as inherited/systemic (like the Skeleton-primitive note), not a story-local regression; escalate to uiux-design-system-builder for a single primitive-level fix.

## Semantic — Boolean status badge used for a tri-state enum, unused i18n copy already exists for the missing state
Pattern: a `StatusBadge` wrapper (`SealStatusBadge`) is typed `sealed: boolean` and a consumer calls it as `sealed={status === "SEALED"}` for an enum that actually has 3 values (PENDING/SEALED/UNSEALED) — collapsing PENDING into the "unsealed" label/aria-label for BOTH sighted and screen-reader users, even though the correct 3-way copy (`termStatus.PENDING/SEALED/UNSEALED`) already exists in `messages/{vi,en}.json` and is simply never wired to any component (grep `termStatus` in `presentation/` returns zero hits). Tell: when a status has ≥3 real states, check every badge call site actually threads the full enum, not a derived boolean — and grep any suspiciously-named i18n keys for zero usages before assuming a binary badge is "good enough."
Fix: give the badge component (or add a sibling one) a `status: <Enum>` prop with per-state tone/icon/label, reusing the already-present i18n namespace instead of deriving a lossy boolean.
Seen in: US-E18.54 seal-status-badge.tsx + academic-record-screen.tsx `TermSection` (`sealed={term.status === "SEALED"}` for all 3 states).

## ARIA — aria-disabled button with no visual disabled state (Button primitive only styles native `disabled`)
Pattern: `components/ui/button/button.tsx`'s variant classes key the dimmed/inert look off `disabled:opacity-50 disabled:pointer-events-none` (the native HTML attribute) — there is no `aria-disabled:` rule. A "coming soon"/blocked action implemented as `<Button aria-disabled="true" title="...">` (kept focusable + a manual `onClick={(e)=>e.preventDefault()}` no-op, per the APG "programmatically disabled but focusable" pattern) therefore renders visually IDENTICAL to a fully enabled button — the only cue is a mouse-hover `title` tooltip (invisible to keyboard-only and touch users) and whatever `aria-disabled` itself announces to AT (partial: no reason given, since `title` isn't wired via `aria-describedby`).
Fix: add `aria-disabled:pointer-events-none aria-disabled:opacity-50` to the specific button's className (or to the shared Button primitive's base class so every `aria-disabled` button in the app gets it for free), and replace/augment `title` with an `aria-describedby`'d `sr-only` span carrying the reason text.
Seen in: US-E18.54 academic-record-screen.tsx print button ("Tính năng in sẽ sớm ra mắt").

## ARIA — Tabs pattern: aria-controls on inactive tabs points to a tabpanel id that isn't in the DOM
Pattern: a roving-tabindex `role="tablist"` sets `aria-controls={`tabpanel-${item.id}`}` on EVERY tab unconditionally, but the consuming screen only ever mounts ONE tabpanel (the active item's) rather than keeping all panels present-but-hidden. Every non-active tab's `aria-controls` therefore references a nonexistent element id — invalid per the ARIA spec and flagged by axe-core's `aria-valid-attr-value` rule once a story renders >1 item.
Fix: only set `aria-controls` on the currently-selected tab (`aria-controls={active ? \`tabpanel-${id}\` : undefined}`), since that's the only panel actually in the DOM at any moment — cheaper than rendering all panels with `hidden`.
Seen in: US-E18.54 year-timeline.tsx (`role="tab"` buttons) + academic-record-screen.tsx (single mounted `role="tabpanel"` for the active year only).
EOF


## Contrast — group-info-panel icon-only buttons keep reusing text-muted-foreground (3rd occurrence, US-E18.51)
Pattern: the NEW unpin (`PinOff`) icon-only button in `pinned-message-row.tsx` copies the exact same `text-muted-foreground` (#8898a9, 2.95:1 on white) base-state icon color already flagged on this same panel's edit-name button and the shared `DetailPanelHeader` back button. Icon-only buttons need 3:1 minimum (WCAG 1.4.11); 2.95:1 fails by a hair.
Fix: `text-edu-text-secondary` (5.48:1) for the icon's default/base state; keep `hover:text-foreground` for hover.
Seen in: US-E18.51 pinned-message-row.tsx:58 (PinOff unpin button).

## ARIA live region — async sub-section swaps role="status" loading for a plain, unannounced error/empty paragraph
Pattern: a panel's own independent async sub-section (e.g. group-info-panel's pin board, fetched separately from group detail) renders `role="status"` only for the loading branch; the error and empty branches that follow are plain `<p>` with no `role`/`aria-live`. When loading→error or loading→empty happens client-side (no page reload), the `role="status"` region unmounts and the replacement text is silent to screen readers — only the transition INTO loading is announced, not the transition OUT of it into error/empty.
Fix: wrap the whole 3-way conditional (loading/error/content-or-empty) in one persistent `aria-live="polite"` container (or add `role="status"` to the error/empty paragraphs too) so every state transition is announced, not just the loading one.
Seen in: US-E18.51 group-info-panel.tsx:304-332 (pinned board loading/error/empty).

## Focus — Unpin control removes its own row from the list post-mutation, no focus handoff
Pattern: clicking the new unpin button (`onUnpin`) fires a mutation whose `onSettled` invalidates the pin-board query; on success the row (including the button that currently holds focus) disappears from the re-rendered list. No code moves focus elsewhere first. Toast (`sonner`) does announce success/failure via its own live region, softening the SR impact, but keyboard focus itself silently drops to `<body>` — same root cause as the reconnect-button pattern already logged (US-E08.6) and the class-log-screen view-transition pattern, now recurring on a delete-like "row removes itself" action.
Fix: before/in the mutation's `onSuccess`, move focus to a stable anchor near the list (e.g. the "pinnedSection" `<h3>`, given `tabIndex={-1}`) instead of letting it fall through to body.
Seen in: US-E18.51 group-info-panel.tsx (onUnpin wiring) + pinned-message-row.tsx (unpin button).

## Focus/live-region — SSR-rendered "dead link" notice states never announced + terminal swap drops focus (US-E18.53)
Pattern: A public unauthenticated entry route (e.g. `/invitations/redeem?token=`) resolves its terminal/error state SERVER-SIDE (RSC) and renders a `role="alert"` block as part of the INITIAL HTML. `role="alert"`/`aria-live` regions only reliably announce on a DYNAMIC DOM *mutation* after mount — content already present at first paint typically gets zero automatic announcement (screen reader is not "told" anything changed). Separately, when the SAME `role="alert"` component (`InvitationNotice`) replaces a client-rendered form after a submit failure (e.g. 409 account-exists, replayed-token 410), the triggering Submit button (which held focus) unmounts — focus drops to `<body>` even though the live-region content itself is now genuinely announced (this half is fine, matches the ARIA-correct case). Net effect: a user who clicks a broken/expired invite link and lands directly on the page gets silence; a user who submits into a terminal failure gets the announcement but loses focus positioning.
Fix: give the shared notice component (or its wrapping container) `tabIndex={-1}` + a `useEffect(() => { ref.current?.focus() }, [])` that fires on every mount — this single fix in the SHARED component covers both the page-load case (focuses + positions on hydration) and the post-submit swap case (focuses on the newly-mounted replacement) without needing two separate call-site fixes. Precedent: same root cause as the already-documented "Interactive element unmounted on its own click handler's state transition" (US-E08.6) and "conditionally unmounting the whole role=status wrapper" entries above — SSR-rendered terminal state is a new variant of the same family (focus/live-region gap on state entry, not just on state exit).
Seen in: US-E18.53 invite-redeem-screen.tsx (5 page-load `vm.kind` states: invalid/expired/rate-limited/tenant-inactive/error) + invitation-notice.tsx (shared component, also used by invite-accept-screen's pre-existing TokenError — same gap was latent there too, just not flagged until this audit).

## Focus — Ternary-swapped CTA buttons across a multi-step mutation flow lose focus every step
Pattern: A `useReducer`-driven flow (idle/ready/confirming/submitting/submitted) renders a DIFFERENT ternary branch's button per state (e.g. "Nộp bài" vs "Hủy"/"Xác nhận nộp" vs a terminal banner with no button) instead of one persistent control. Every state transition unmounts the focused button and mounts a new one with no `useRef`+`useEffect` to re-target focus — content is still announced (role="status"/"alert" on the new subtree), but a keyboard user's visible focus silently reverts to `<body>` after every click, on the exact flow (one-way/irreversible submit) where staying oriented matters most.
Fix: `useRef` on the button/heading each transition lands on; `useEffect` keyed on the reducer's status field calls `.focus()`. For terminal states that replace the whole control with a banner, give the banner root `tabIndex={-1}` + a ref and focus it on mount.
Seen in: US-E24.5 submit-box.tsx (course player assignment submit flow) — same class as US-E13.3's "View transitions (SPA-style) without focus management", now recurring in a useReducer-shaped confirm/submit flow rather than a view-toggle.

## Contrast — Sub-tab count pill: primary-accessible text on primary-light tint fails at small extrabold size
Pattern: active sub-tab's count badge uses `bg-edu-primary-light text-edu-primary-accessible` at 10.5px font-extrabold. `--edu-primary-accessible` (#4468E0) passes on WHITE (4.88:1, well-established) but on the lighter `--edu-primary-light` tint (#ECF2FF) drops to 4.35:1 — fails 4.5:1 (10.5px extrabold doesn't reach the ≥14px-bold large-text threshold). Root cause: assuming a token's "passes on white" result transfers unchanged to a tinted background — always recompute per actual adjacent bg.
Fix: swap to `text-foreground` (#2A3547, 11.5:1 on the tint) — same fix already used by StatusBadge for primary/info/purple/teal tones.
Seen in: US-E24.4 cross-subject-list.tsx (sub-tab count badge, active state).

## ARIA pattern — Link-based `role="tab"` (URL-is-state tab strip) intentionally has no arrow-key/roving-tabindex
Pattern: several tab strips in this repo are real per-view `<Link>`s with `role="tab"`/`aria-selected` for STATIC correctness, but with NO arrow-key handler or roving `tabIndex` (every tab stays in normal Tab order) — because each "tab" is actually a distinct route/page, not a JS-toggled panel. This deliberately diverges from the full ARIA APG Tabs keyboard pattern (which expects Home/End/Arrow to move focus, only the active tab Tab-reachable).
Assessment: NOT a defect to fix per-screen — established, repeated, accepted convention (`class-hub-tabs.tsx` US-E24.8, `cross-subject-list.tsx` US-E24.4). Tab/Shift+Tab/Enter fully satisfy 2.1.1 keyboard operability; a true widget's roving-tabindex pattern (like `scope-tabs.tsx`'s client-state ArrowLeft/Right) is reserved for tab strips that toggle a panel via `useState`, not real navigation. Only raise as a design-system follow-up (shared `LinkTabList` primitive) if the team wants full APG parity everywhere — don't block an individual story for it.
Seen in: US-E24.4 cross-subject-list.tsx (advisory-only, A11Y-002).

## Focus — list item removes itself after a successful mutation, no focus handoff (2nd confirmed site)
Pattern: an inbox-style list (row = student request) where a successful Approve/Reject action filters the acted-on row out of local state. The button that was focused (or, for a dialog flow, the button `useDialogReturnFocus`/`useDialogReturnFocusPrimitive` captured as the invoker) is unmounted in the same render as the dialog closing — so Radix's `onCloseAutoFocus` calls `.focus()` on a now-detached DOM node (silent no-op) and native focus drops to `<body>`. Root cause is structural: "remove decided row" UX + "focus follows the row's own button" are incompatible without an explicit handoff.
Fix: on the SAME state update that filters the row, move focus to a stable anchor first — e.g. the card's `<h3>` (`tabIndex={-1}` + `.focus()`) or the next/previous row's first button, via a `ref`+`useEffect` keyed on the list length or the removed id. See US-E08.6 precedent for the general "own-click unmounts own element" pattern; this is the list-row variant of it, worth checking on every future approve/reject/dismiss-row list.
Seen in: US-E24.11 pending-leave-card.tsx (both instant Approve and dialog-driven Reject paths).

## ARIA — StatusBadge count using `aria-label` prop recurs even though the fix is already documented in-repo
Pattern: `<StatusBadge tone=... aria-label={t("countLabel", {count})}>{count}</StatusBadge>` — bare digit as visible children, full sentence only via the `aria-label` prop. `StatusBadge`/`Badge` renders a plain `<span>` (ARIA role=generic, "Name: prohibited" per ARIA 1.2) — the established, ALREADY-DOCUMENTED-IN-CODE fix (`kpi-tile.tsx`'s `isDemo` badge, comment tag A11Y-003) is: render the short visible text with `aria-hidden="true"` and add a sibling `<span className="sr-only">{fullLabel}</span>` as the badge's CHILDREN, never rely on the `aria-label` prop alone. New call sites keep reintroducing the unreliable form because `StatusBadge`'s type signature still accepts (and its own JSDoc doesn't warn against) a bare `aria-label` prop.
Fix at the call site: swap to `<StatusBadge tone=...><span aria-hidden="true">{count}</span><span className="sr-only">{t("countLabel", {count})}</span></StatusBadge>`. Longer-term: consider flagging `StatusBadge`'s `aria-label` prop as JSDoc-deprecated/warned against, since every new consumer defaults to it.
Seen in: US-E24.11 open-violations-card.tsx, pending-leave-card.tsx (both count badges). Precedent fix: teacher-classes-screen kpi-tile.tsx (already correct).
