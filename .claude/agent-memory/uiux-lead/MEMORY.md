...
- [DR-023 scoped-extension pattern](dr-023-scoped-extension-pattern.md) — audit-trail as sub-section in existing dialog (not new screen), honest empty-state-dominant seeding for retrofitted trails, mock-first entry-shape discipline, `git branch -d` false-fail gotcha (push --delete remote first)
