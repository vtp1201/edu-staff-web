...
US-E09.6 Student Absences (teacher/GVCN record+edit own-class, principal schoolwide flag-only) — implemented 2026-07-25, merged 4b53a1d.
- Genuinely independent domain: 2-state ONE-WAY `RECORDED→FLAGGED_UNEXCUSED` (NOT the shared `ApprovalTransition` DRAFT/SUBMITTED/APPROVED/REJECTED shape US-E09.5 uses) — confirmed zero import of that shape anywhere in the feature by the reviewer.
- **Role-discriminated VM as a compile-time security guarantee**: the engineer tightened the VM beyond the architecture doc into a discriminated union on `viewerRole` — the principal arm has literally no `recordAbsenceAction`/`editAbsenceAction` field, so AC-006.5 (zero record/edit affordance) is unbreakable at the type level, not just a runtime check. Worth reusing as the default pattern whenever a role has a hard "never do X" requirement.
- Constructor-injected `StudentAbsenceAuthContext` (role+memberId+classId) into the mock repo = the authorization boundary; every mutating method checks it as the FIRST statement, before any existence check (so forbidden never leaks "exists vs not" info). This `authCtx`-injection pattern is now its 3rd instance (parent-links, staff-discipline, student-absences) — tech-lead-reviewer flagged (again) that it's ADR-worthy as a named convention rather than 3 parallel precedents; still no ADR written.
- **`initialData` is TanStack Query-observer-scoped, not query-key-scoped** — the engineer found and fixed a real bug during implementation: seeding `initialData` unconditionally on every render re-seeds it on every filter change, so a changed class/date filter never actually refetches (would have silently broken AC-002.2). Fix: only seed `initialData` for the RSC's own initial filter value, not on every filter change. Flag this explicitly in any future story using `initialData` + a client-side-mutable filter.
- **`useDialogReturnFocus` must be wired on every controlled AlertDialog/Dialog with no `<Trigger>` element** — a11y audit caught a real WCAG 2.4.3 blocker: `components/shared/publish-confirm-dialog` (shared by 3 features) was invoked as `open={x}` with no `<AlertDialogTrigger>`, so Radix's default `onCloseAutoFocus` had no trigger ref and focus silently dropped to `<body>` on Cancel/Escape. Fixed once in the shared component (benefits all 3 consumers). `destructive-confirm-dialog.tsx` already had this fix — check for it whenever adding a NEW controlled-dialog-no-trigger shared component.
- Pipeline ran full 8-agent loop (planner → architect+state-engineer parallel → engineer → reviewer+a11y parallel → fix pass → QA) with ONE revision round: tech-lead found 1 MUST-FIX (flag mutation's `not-found` branch closed the confirm dialog with zero user feedback — looks identical to success on an irreversible action) + 4 SHOULD-FIX; a11y found 1 blocking (dialog focus-restore above) + 1 major (dead-authored i18n success-toast keys never wired) + 2 minor (deferred). All fixed in one consolidated engineer pass, re-verified, QA gave Conditional Pass (100% AC coverage, closed one self-found gap — `listAbsences`'s deny-by-default branch for non-teacher/non-principal roles had zero test coverage anywhere in the suite).
- Final: 118 unit/integration (feature-scoped) + 31 Storybook stories; full suite 417 files/2776 tests + 147 files/1025 Storybook tests; tsc clean; build green (both routes present, confirmed no `/admin/absences`).
