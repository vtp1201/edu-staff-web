...
- **shadcn/ui primitives** live one-per-folder under `components/ui/<name>/` and are added via `bun ui:add <name>` (never hand-copied). Composed components go in `components/shared/` or `features/<x>/presentation/`. Flag any missing primitive — do NOT plan to hand-write a Radix primitive.
