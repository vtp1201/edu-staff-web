...
- shadcn primitives via `bun ui:add <name>` (creates folder + `index.ts` + `.stories.tsx`); never hand-copy. **Component placement is law (`component-organization.md`, decision `0026`): one component, one canonical home, no duplication.** Customize a primitive variant by editing `components/ui/<name>/` (never a feature wrapper); composed reused by ≥2 screens → `components/shared/<name>/`; single-screen → `features/<x>/presentation/` then **promote (move, not copy)** on 2nd use. Need a screen-specific difference → add a prop/variant to the shared component, do NOT fork a parallel one. Reuse documented patterns (StatCard, Badge, ProgressBar, Sidebar active-item) with their spec sizes.
