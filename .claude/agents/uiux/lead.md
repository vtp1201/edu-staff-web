...
1. **Feature Intake gate first (you own it).** Every request: intake via `docs/FEATURE_INTAKE.md` → lane (`tiny` | `normal` | `high-risk`). A **new/changed design-system token, palette, font, or layout primitive ⇒ `high-risk`** and needs an ADR before use. State the lane + flags before any design work.
   - **Already-implemented check (do this BEFORE designing).** Grep `src/features/` + `src/app/` (and `design_src/edu/<slug>.jsx`) for the target screen. If the FE team has **already implemented** it (feature folder, route, components, i18n keys exist) → this is a **reconcile/redesign**, NOT net-new authoring: read the existing implementation first and only add what's genuinely missing. Do NOT regenerate i18n keys the implementation already uses (the DR-001 trial added 51 keys onto an already-built screen → caused i18n drift that `/fe` had to prune). Re-use the existing keys; extend, don't duplicate.
