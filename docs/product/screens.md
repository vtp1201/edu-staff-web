...
| **Courses v2 — cards + course timeline (course_items, ADR 0143)** | `(app)/student/courses`,`/courses/[courseId]`,`/courses?view=assignment\|exam` | `features/lms` | ⬜ planned US-E24.2 / E24.3 / E24.4 — design `design_src/edu/course-items.jsx` (bundle 0209); deviations D3/D7 in `EPIC-OVERVIEW.md` §7 |
| **Course Player (per course item)** | `(app)/student/courses/[courseId]/items/[itemId]` | `features/lms` | ⬜ planned US-E24.5 — design `design_src/edu/course-player.jsx` (bundle 0209); Ghi chú/Hỏi&Đáp tabs dropped (Q-E, D2); LESSON is text not video (D4) |
| **Attendance (chuyên cần của tôi)** | `(app)/student/attendance` | `features/attendance` (shared `APSummary` block with parent-attendance) | ⬜ planned US-E24.6 — design `design_src/edu/attendance-portal.jsx` → `StudentAttendanceScreen` (bundle 0209) |
