...
| Assignments | `(app)/student/assignments` | `features/lms` | ✅ US-E11.7, **re-pointed to the REAL `lms` contract by US-E24.1 (ADR 0075)** — standard `USE_MOCK` gate restored. Flat class-scoped list (title + deadline); the 4 status tabs, the graded sheet, the file attachment and the late-submit confirm were DELETED (the list row carries no per-student status, grading is BE US-141, a submission is `{content}` only, and a late submit is now refused with `409 LMS_ITEM_CLOSED`). Submission state resolves on sheet-open via `.../submissions/me` — ⚠️ US-E24.4 merges this into the unified "Khoá học" cross-cutting tab (`/student/courses?view=assignment`); this sidebar route becomes a permanent redirect (Q-C) |
| Exams (list/briefing/taking/result) | `(app)/student/exams`,`/exams/[id]` | `features/exam` | ✅ US-E11.1 (base); ⬜ US-E11.5 (mixed MCQ+essay variant) — ⚠️ US-E24.4 merges the LIST into the unified "Khoá học" cross-cutting tab (`/student/courses?view=exam`); `/exams/[examId]` (briefing/taking/result) stays as-is (Q-C). Route/status unchanged until E24.4 lands |
| **Courses v2 — cards + course timeline (course_items, ADR 0143)** | `(app)/student/courses`,`/courses/[courseId]`,`/courses?view=assignment\|exam` | `features/lms` | ⬜ planned US-E24.2 / E24.3 / E24.4 — design `design_src/edu/course-items.jsx` (bundle 0209 v3); D7 now normative (only EXAM rows render 'Sắp mở' for students), remaining real gap: text/link-only submission, no file upload (ask #1) |
