...
| Classes / Class roster | `(app)/teacher/classes`,`/classes/:classId/students` | `features/teacher` | ✅ US-E13.1 — → Class Hub redesign planned, US-E24.7/E24.8 (`/classes/:classId/students` becomes the `?tab=students` tab of the hub; route stays until then) |
...
| **Course Player (per course item)** | `(app)/student/courses/[courseId]/items/[itemId]` | `features/lms` | ⬜ planned US-E24.5 — design `design_src/edu/course-player.jsx` (bundle 0209 v3); Ghi chú/Hỏi&Đáp tabs dropped (Q-E, D2, normative) — only Tổng quan; LESSON is text not video (D4, normative); remaining real gaps: file-upload submission (ask #1), grade/feedback shown only when BE returns it (ask #2) |
