...
| Redeem tenant invitation — self-serve registration (public, no shell) | `/invitations/redeem?token=...` | `features/auth/presentation/invite-redeem` | ✅ (US-E18.53, IAM US-191 / ADR `0130`/`0131` — AMENDS ADR `0059`'s "no guest account-creation": BE now creates account + ACTIVE membership + tenant-scoped session in one public call. Preview via `POST /invitations/lookup`, submit via `POST /invitations/redeem`; a 409 `INVITATION_ACCOUNT_EXISTS` hands off to the accept flow above. Same `invitations.jsx` shell/tone-only caveat. **US-E18.59 / ADR `0072`:** both calls are now issued DIRECTLY FROM THE BROWSER so IAM's per-IP rate limit sees each visitor's own IP — this adds a NEW visible `loading` state (skeleton card, `role="status"` + `aria-live="polite"`, copy `invitations.redeem.states.loading`) that the previously server-rendered preview never had; a blank token still renders the dead-link card with zero network) |
...
| Messaging (inbox + 1:1 + group) | `(app)/(shared)/messages` | `features/messaging` | ✅ US-E10.1/E10.4/E10.5 (BE hybrid US-E18.17/ADR 0060 — group lifecycle/pin mock; contacts REAL từ US-E18.52 qua IAM narrowed tier) |
...
| Classes / Class roster | `(app)/teacher/classes`,`/classes/:classId/students` | `features/teacher` | ✅ US-E13.1 → superseded by Class Hub (US-E24.7/E24.8): `/classes/:classId/students` is now the `?tab=students` tab of the hub (old route 308-redirects) |
| **Class Hub (role-aware class list + class detail shell)** | `(app)/teacher/classes`,`/classes/[classId]?tab=students\|timetable\|course\|homeroom` | `features/teacher` (+ `class-log`, `lms`, `discipline` tabs) | ⬜ planned US-E24.7 (list) / E24.8 (shell + deep-link) / E24.9 (Thời khoá biểu + sổ đầu bài) / E24.10 (Khoá học online) / E24.11 (Chủ nhiệm) — design `design_src/edu/class-hub.jsx` (bundle 0209 v3); D1/D5 now normative (2 sổ: nhật ký GVCN 4 trạng thái + ghi sổ/chuẩn bị tiết GVBM), remaining real gap: GVCN readonly môn khác (ask #7) |
...
| Grade Book (principal read + reject) | `(app)/principal/grades` | `gradebook.jsx` | `features/grades` | ✅ US-E13.6 — route thật `(app)/principal/grade-book` (drift so với design route); từ US-E18.44 render entry-side grid (`viewerRole: "approver"` — xem, không sửa điểm, được reject cell `PENDING_APPROVAL`) thay vì read-only `GradeBookRow` cũ; nav entry thêm mới (route trước đó orphan) Từ US-E18.46 thêm khu "Đang chờ duyệt" (rollup toàn tenant `GET /grade-entries/pending-approval`, cũ→mới, load-more) ngay trên picker: bấm 1 dòng nhảy thẳng tới đúng lớp-môn-học kỳ; cell `PENDING_APPROVAL` có thêm nút Duyệt (approve, xác nhận không cần lý do). |
...
| **Grade Book (admin read + reject)** | `(app)/admin/grade-book` | `gradebook.jsx` (reuse) | `features/grades` | ✅ US-E13.6 (reuse of `principal/grade-book`'s screen); từ US-E18.44 render entry-side grid (`viewerRole: "approver"`, view + reject `PENDING_APPROVAL`, no score edit) thay vì read-only `GradeBookRow` cũ (BE US-184); nav entry thêm mới (route trước đó orphan) Từ US-E18.46 thêm khu "Đang chờ duyệt" (rollup toàn tenant `GET /grade-entries/pending-approval`, cũ→mới, load-more) ngay trên picker: bấm 1 dòng nhảy thẳng tới đúng lớp-môn-học kỳ; cell `PENDING_APPROVAL` có thêm nút Duyệt (approve, xác nhận không cần lý do). |
| **Timetable Builder** | `(app)/admin/timetable` | `timetable.jsx` (US-045) | `features/admin/timetable` | ✅ US-E12.5 (BE US-E18.11 grid CRUD) + **US-E18.48**: NEW whole-school conflicts panel ("Xung đột toàn trường", real `GET /timetable/conflicts?termId=`, BE US-188) REPLACES the old mock-only teacher-clash summary — 2 kinds rendered distinctly (teacher = rejected on write; **room = detected on read only, ADR 0128, never blocked on save**), `truncated` hint, in-panel error degrade, per-cell highlight now real. ADMIN-only surface (`evaluateAdminAccess` strict equality; MANAGER→appRole `principal` is redirected). No handoff mockup for the panel — normative values in `design-spec.jsonc` §`timetableConflictScanPanel`. Ask #16 closed |
...
| Courses + course timeline | `(app)/student/courses`,`/courses/[courseId]` | `features/lms` | ✅ US-E11.6, **re-pointed to the REAL `lms` contract by US-E24.1 (ADR 0075, supersedes 0073)** — ask #51 closed, standard `USE_MOCK` gate restored. Grid = title + publish status; detail = the `course_items` timeline + a lazy plain-text lesson reader. The progress tabs, chapter list, video/PDF players, notes, Q&A and mark-complete were DELETED (no wire backing; completion/progress is BE US-254 draft, ADR 0076) — ⚠️ US-E24.2 (course cards v2) + US-E24.3 (timeline design) + US-E24.5 (`/courses/[courseId]/items/[itemId]`) still to come |
| Assignments (redirect) | `(app)/student/assignments` | — | 🔁 **Permanent redirect (308) since US-E24.4** → `/student/courses?view=assignment`. The standalone screen (`features/lms/presentation/student-assignments/**`) and its Server Actions are DELETED: the class's assignments are now the cross-subject view of the courses screen, composed from the real `lms` course timelines (US-E24.1/E24.2). Sidebar entry removed |
| Exams — list (redirect) | `(app)/student/exams` | — | 🔁 **Permanent redirect (308) since US-E24.4** → `/student/courses?view=exam`. The `exam-list` presentation (and its mock-era `MOCK_STUDENT_ID` read) is DELETED. Sidebar entry removed |
| Exams — briefing/taking/result | `(app)/student/exams/[examId]` | `features/exam` | ✅ US-E11.1 (base); ⬜ US-E11.5 (mixed MCQ+essay variant). UNCHANGED by US-E24.4 — this is where an open exam's "Vào làm bài" CTA lands (`makeListExamsUseCase`; core has no single-exam GET) |
| **Courses v2 — cards + course timeline (course_items, ADR 0143)** | `(app)/student/courses`,`/courses/[courseId]`,`/courses?view=assignment\|exam` | `features/lms` | ✅ US-E24.2 (cards) / E24.3 (timeline) / E24.5 (player) / **E24.4 (cross-subject `?view=`/`?sub=` tabs — assignments + exams merged in, old routes redirect)** — design `design_src/edu/course-items.jsx` (bundle 0209 v3); D7 normative (only EXAM rows render 'Sắp mở' for students); remaining real gaps: text/link-only submission, no file upload (ask #1) and no per-student submission flag on a timeline item (D-1, extended to the cross-subject row by E24.4) |
| **Course Player (per course item)** | `(app)/student/courses/[courseId]/items/[itemId]` | `features/lms` | ✅ implemented US-E24.5 (high-risk lane; allowlisted iframe embeds, single-attempt text submit) — design `design_src/edu/course-player.jsx` (bundle 0209 v3); Ghi chú/Hỏi&Đáp tabs dropped (Q-E, D2, normative) — only Tổng quan; LESSON is text not video (D4, normative); remaining real gaps: file-upload submission (ask #1), grade/feedback shown only when BE returns it (ask #2) |
| **Attendance (chuyên cần của tôi)** | `(app)/student/attendance` | `features/attendance` (shared `components/shared/attendance-summary` block with parent-attendance) | ✅ US-E24.6 — real `GET members/{me}/attendance` (memberId claim only, ADR 0074); term range from the active academic year with a 6-month fallback when the admin-shaped calendar read is refused; "buổi/ngày" vocabulary, not "tiết" |
...
| Attendance (per-child history, date-range) | `(app)/parent/attendance` | `features/parent-attendance` (new; + promoted shared `components/shared/child-switcher/`) | ✅ US-E20.5 (was a dead sidebar link — full 404; mock-first, BE `GET /members/{id}/attendance` doesn't authorize PARENT — new cross-repo ask; reuses ADR-0058's 4-state status vocabulary) — ✅ US-E24.6 added the shared `APSummary` block + the "Xin phép nghỉ" dialog (`components/shared/leave-request-dialog/`, D6 normative: from/to date pair, no per-period scope, ≤3 attachments jpg/png/pdf ≤5MB via the DEPLOYED core US-249 route). `classId` for the request is read off the child's most recent attendance row (no PARENT-callable enrolment endpoint); an empty range disables the button with a visible reason |
...
- **Design bundle 0209 (2026-09-02, US-E24.0 — selective sync, epic E24)** adds:
  - `design_src/edu/class-hub.jsx` — Teacher Class Hub: role-aware class list + class detail shell with 4 tabs (US-E24.7..E24.11, net-new)
  - `design_src/edu/course-items.jsx` — `StudentCoursesV2` + `CourseTimelinePage` (student/teacher/readonly modes) + `CrossSubjectList` (US-E24.2/E24.3/E24.4, net-new)
  - `design_src/edu/course-player.jsx` — `CourseItemPlayer`, Udemy-style content pane + course-content sidebar (US-E24.5, net-new)
  - `design_src/edu/attendance-portal.jsx` — `StudentAttendanceScreen` + `ParentAttendanceScreen` + `APExcuseRequestDialog` (US-E24.6, net-new)
  - updated in place: `app.jsx` (navParam deep-link, dark mode), `ui.jsx` (sidebar/footer/bell/avatar dropdown), `teacher.jsx`, `student.jsx`, `classops.jsx`, `messaging.jsx`, `academic-record-view.jsx`, `icons.jsx` (+`helpCircle`/`moon`/`maximize`), plus `EduPortal.html`, `CHANGELOG.md`, `README.md`
  - NOT taken from the bundle: `tokens.js` (comment-only diff — repo comments cite decisions 0040/0027/0046/0013), `parent-links.jsx` (bundle drops the DR-023 audit trail — EPIC-OVERVIEW §7 D8), and the 5 uiux-authored files the bundle never shipped (`assignments.jsx`, `lesson-plan.jsx`, `question-bank.jsx`, `staff-discipline.jsx`, `student-absences.jsx`)
  - known deviations D1–D9 (mockup vs shipped BE contract) live in `docs/stories/epics/E24-learning-class-hub/EPIC-OVERVIEW.md` §7 and are flagged per screen in `design-spec.jsonc`
- **Design bundle 0209 v3 (2026-09-02, US-E24.0b — D1–D9 now match the BE contract + R1–R3)** supersedes the v1 sync above:
  - `class-hub.jsx`, `course-items.jsx`, `course-player.jsx`, `attendance-portal.jsx` re-copied (v1 file already existed; v3 replaces content) — D1/D2/D3/D4/D5/D6/D7/D9 are now NORMATIVE layout, not "chờ bundle sửa"; only 4 real gaps remain: GVCN readonly môn khác (ask #7), tạo EXAM từ timeline (ask #6), upload file bài nộp (ask #1), điểm bài nộp hiển thị khi BE trả về (ask #2)
  - **`parent-links.jsx` copied this time** (v1 skipped it, D8) — v3 independently restores `PLAuditTrailSection` inside `PLDetailDialog` (same placement DR-023 specified) + adds `consent_agreed`/`consent_declined` actions; see DR-023 §Status note (mockup superseded, shipped FE code unchanged)
  - `tokens.js` merged by hand — kept the repo's decision comments (0040/0027/0046/0013), added the 5 new keys (`successText`, `tealText`, `mediaSurface`, `chipBg`, `inputBg`) + the `T_LIGHT`/`T_DARK`/`applyTheme` dark-mode block (R2); all values byte-identical to the bundle, no new runtime token, no ADR
  - root `CHANGELOG.md` re-copied (R1–R3 + D1–D9 bundle notes); `EduPortal.html`/`README.md` were already byte-identical
  - still kept, unchanged: the 5 DR-020..022 files (`assignments`, `lesson-plan`, `question-bank`, `staff-discipline`, `student-absences.jsx`) — bundle still does not ship them
