...
| **Attendance (chuyên cần của tôi)** | `(app)/student/attendance` | `features/attendance` (shared `APSummary` block with parent-attendance) | ⬜ planned US-E24.6 — design `design_src/edu/attendance-portal.jsx` → `StudentAttendanceScreen` (bundle 0209 v3) |
...
| Attendance (per-child history, date-range) | `(app)/parent/attendance` | `features/parent-attendance` (new; + promoted shared `components/shared/child-switcher/`) | ✅ US-E20.5 (was a dead sidebar link — full 404; mock-first, BE `GET /members/{id}/attendance` doesn't authorize PARENT — new cross-repo ask; reuses ADR-0058's 4-state status vocabulary) — US-E24.6 adds the "Xin phép nghỉ" dialog (`design_src/edu/attendance-portal.jsx` → `ParentAttendanceScreen`/`APExcuseRequestDialog`, bundle 0209 v3); D6 now normative (from/to date pair, no per-period scope, +3 attachments jpg/png/pdf ≤5MB, draft US-249) |
