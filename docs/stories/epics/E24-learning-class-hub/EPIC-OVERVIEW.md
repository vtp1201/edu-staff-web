...
| **E24.11** Tab Chủ nhiệm (GVCN) | Điểm danh hôm nay (3 số + "Mở sổ điểm danh"), vi phạm chờ xử lý (+link Discipline), đơn xin nghỉ Duyệt/Từ chối (đính kèm ≤3 file — US-249, **đã deployed trong `openapi.yaml`**, không còn draft) | `GET classes/{id}/attendance?date`, `GET conduct/student-violations`, `student-leave-requests` +approve/reject (real) | Gỡ force-mock leave trong `discipline.di.ts` cho nhánh này |
