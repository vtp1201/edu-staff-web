...
| US-E18.12 | Grades contract remap | **rất cao** | high-risk | **In progress** — ground-truthed against `core`'s `GradeEntry`/`GradeReport` tags + Go source. Confirms `classSubjectId`/batch ≠ real `(classId,subjectId,termId)` + per-cell status (not per-row). Teacher entry/submit + multi-role read (incl. student self + parent-linked via `/members/{id}/grades`) wired real. `IGradeApprovalRepository` (admin cross-class batch dashboard) + parent child-switcher stay **permanently mock** — no wire batchId/rollup/display-name source, and no reject transition exists for `GradeEntry` at all. ADR `0054`. See `US-E18.12-grades-wiring/story.md`. |
