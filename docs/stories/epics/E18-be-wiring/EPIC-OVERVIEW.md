...
| Story | Title | Drift | Lane | Ghi chú |
|-------|-------|-------|------|---------|
| US-E18.50 | Messaging group room lifecycle PARTIALLY real (social US-193, ADR 0132) | cao | normal | **Done** 2026-08-07 — ground-truth từng method trong 7 method của group slice: đúng HAI method thành real — `createGroup` → `POST /rooms/groups` (body `{name}` ONLY ⇒ form create thu về một bước chỉ-tên, `CreateGroupInput` narrowed, xoá color-swatch picker) và `deleteGroup` → `POST /rooms/{roomId}/archive` (204, idempotent, soft archive). 5 method còn lại VẪN mock với lý do per-method ghi cạnh stub (getGroup/add/remove cần `GroupEntity` có display name mà wire không có; updateGroup không có endpoint; leaveGroup self-leave không scope `custom` — wire vào sẽ cho member rời class_chat hệ thống). Failure mới `create-group-forbidden` (403 allow-list ADMIN/MANAGER/TEACHER/STAFF) + `group-not-self-service` (409 room hệ thống), branch theo CODE; affordance create role-gated server-derived (`getSessionRole` → `canCreateGroupFor`, fail-closed) nên STUDENT/PARENT không thấy nút chỉ-có-thể-403. Review PASS + a11y PASS (2 minor ghi trong packet). Closes **#32(a)**. See `US-E18.50-messaging-group-lifecycle-real/US-E18.50-messaging-group-lifecycle-real.md`. |
...
| US-E18.53 | Self-serve registration qua invitation redeem (iam US-191, ADR 0130/0131 → FE ADR 0071 amends 0059) | cao | high-risk | **Done** 2026-08-07 — flow public MỚI `/[locale]/invitations/redeem`: `POST /invitations/lookup` (public, preview `{email, tenantName, role}` để build form) → form password+fullName (KHÔNG có field email — email lấy từ invitation) → `POST /invitations/redeem` (201 trả member + tenant-scoped session NGAY, khỏi signin lại) → `setAuthCookies` dùng chung (không fork) → redirect derive CHỈ từ `member.tenantId`+`roles[0]` (không nhận `next`/`returnTo` từ client). Token CHỈ đi trong POST body — có test assert URL serialize không chứa token; DI dùng bare `createHttpClient()` không bearer (chống confused-deputy trên máy dùng chung). 410 = token hết hạn/đã dùng/replay (terminal, không gợi retry); 409 `INVITATION_ACCOUNT_EXISTS` → CTA route sang sign-in + accept flow có sẵn. `InvitationNotice` PROMOTE (move, không copy) từ `invite-accept-screen` sang `components/shared/` (decision 0026). High-risk gates đủ: tech-lead **Approved** (security focus, 0 blocking) + a11y **PASS** + ADR **0071** đăng ký (amend 0059 — kết luận "no guest account-creation" hết hiệu lực cho redeem, accept flow giữ nguyên). Ask BE mới: rate-limit 10/min per-IP đang bucket theo IP của Next server (không forward XFF) — cần quyết platform-wide. Closes **#31**. See `US-E18.53-invitation-redeem-registration/US-E18.53-invitation-redeem-registration.md`. |
