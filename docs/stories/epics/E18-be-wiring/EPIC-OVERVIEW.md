...
| US-E18.28 | Exam-bank edit/delete + MCQ options wiring (core US-152) | cao | normal | **In progress** — ground-truthed `core`'s US-152 additions on `origin/main` 2026-08-01: `PATCH`/`DELETE /exam-papers/:id` (author-only, DRAFT-only) + `PUT`/`DELETE /exam-papers/:id/questions/:questionId` + `options`/`correctOptionId`/`difficulty` on `AddQuestionRequest`/new `UpdateExamQuestionRequest`/`ExamQuestionResponse`. Closes cross-repo asks **#24**/**#25**/**#26** (RESOLVED, `createExam`'s bulk-create half of #26 stays open — no wire equivalent, unchanged). See `US-E18.28-exam-bank-edit-delete-wiring/story.md` + ADR `0056` Amendment 2. |
