...
import { MOCK_STUDENT_ACADEMIC_RECORDS, MOCK_SUBJECT_NAMES } from "./fixtures";
 ...
 * becomes a year (US-E18.54) — including the denormalized `academicYear` each
 * fixture row now carries (US-E18.56). Only the subject-name decoration is a
 * local map here instead of an HTTP read.
 */
  ...
  implements IAcademicRecordsRepository
{
  async getRecords(_memberId: string): Promise<AcademicRecordResult> {
    ...
    return {
      ok: true,
      data: buildAcademicRecord(dto.studentMemberId, rows),
    };
  }
}
