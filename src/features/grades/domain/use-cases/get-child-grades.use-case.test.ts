...
describe("GetChildGradesUseCase", () => {
  it("returns the child's grade books on success", async () => {
    const getChildGrades = vi.fn().mockResolvedValue([book]);
    const uc = new GetChildGradesUseCase(makeRepo({ getChildGrades }));
    const result = await uc.execute("child-1", "2025-2026");
    expect(getChildGrades).toHaveBeenCalledWith("child-1", "2025-2026");
  ...
  it("maps a thrown forbidden failure", async () => {
    ...
    const uc = new GetChildGradesUseCase(makeRepo({ getChildGrades }));
    expect(await uc.execute("child-x", "2025-2026")).toEqual({
      type: "forbidden",
    });
  });
  ...
  it("maps a generic error to network-error", async () => {
    ...
    const uc = new GetChildGradesUseCase(makeRepo({ getChildGrades }));
    expect(await uc.execute("child-1", "2025-2026")).toEqual({
      type: "network-error",
    });
  });
});
