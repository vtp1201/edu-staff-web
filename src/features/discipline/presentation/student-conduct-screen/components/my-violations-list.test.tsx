...
import { MyViolationsList } from "./my-violations-list";

/**
 * US-E17.11 / AC-E17.11-10 — touch-target regression guard for the read-only
 * student "my violations" list. This list has NO delete/edit icon buttons
 * (parents/students only view), so only the row-height requirement applies.
 *
...
function renderList(): string {
  return renderToStaticMarkup(
    <NextIntlClientProvider locale="vi" messages={messages}>
      <MyViolationsList violations={MOCK_MY_VIOLATIONS} />
    </NextIntlClientProvider>,
  );
}

describe("MyViolationsList — touch target 44px (US-E17.11)", () => {
