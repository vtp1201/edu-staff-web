...
export class CreateTermUseCase {
  ...
  async execute(input: CreateTermInput): Promise<Result<Term>> {
    const { yearId, name, startDate, endDate, existingTerms } = input;
    ...
    const overlaps = existingTerms.some(
      (t) => startDate < t.endDate && endDate > t.startDate,
    );
    if (overlaps) {
      return fail({
        type: "date-overlap",
    ...
    const term = await this.repo.createTerm(yearId, {
      name,
      startDate,
      endDate,
    });
