/**
 * Pure skeleton markup for the post-login select-tenant screen (NFR-003 —
 * never a blank screen while the membership list resolves). Decorative shimmer
 * (`aria-hidden`) + one sr-only aria-live status for screen readers. Kept as a
 * plain presentational component (no directive, no server-only) so `loading.tsx`
 * (RSC) can feed it a translated label and Storybook can render it in isolation.
 */
export function SelectTenantSkeleton({
  loadingLabel,
}: {
  loadingLabel: string;
}) {
